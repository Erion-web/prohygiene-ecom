...
export function CustomerDetailPanel({
  ...
  const city = profile?.city ?? leaseClient?.city
        ...
        <div className="space-y-2.5 text-sm">
          {[
            ...
            <div key={row.label} className="flex items-center justify-between gap-3">
              ...
              <span className="font-medium text-text-primary text-right truncate max-w-[55%]">{row.value}</span>
            </div>
          ))}
        </div>
        ...
        {leaseClient && (
          <div className="admin-card">
            ...
            <div className="space-y-2 text-sm">
              ...
              {(leaseClient.city || leaseClient.address) && (
                <div className="flex items-center gap-2 text-text-secondary">
                  <MapPin size={14} />
                  {[leaseClient.city, leaseClient.address].filter(Boolean).join(', ')}
                </div>
              )}
