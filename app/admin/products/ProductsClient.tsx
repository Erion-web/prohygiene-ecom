...
import {
  Edit, Package, AlertTriangle, Search, X, Star, Tag,
  TrendingUp, ChevronDown, SlidersHorizontal,
} from 'lucide-react'
...
export function ProductsClient({ products, categories, brands }: Props) {
  ...
  const hasDropdownFilters = categoryId || brandId || audience || status || stock
  ...
  const statCards = [
    ...
    { label: 'I ulët',     value: stats.lowStock,   color: 'text-amber-500',   active: stock === 'low',     onClick: () => setStock(s => s === 'low' ? '' : 'low') },
    { label: 'Zbritje',    value: stats.onSale,     color: 'text-brand-600',   active: onSale,              onClick: () => setOnSale(v => !v) },
  ] as const
        ...
        {statCards.map(s => (
          <button
            ...
            className={`bg-white border rounded-xl px-2 py-2.5 text-center transition-all active:scale-[0.97] ${
              s.active ? 'border-brand-400 ring-1 ring-brand-300 bg-brand-50/30' : 'border-gray-100 hover:border-gray-200 hover:shadow-sm'
            }`}
          >
            <p className={`text-lg md:text-xl font-black tabular-nums ${s.color}`}>{s.value}</p>
            <p className="text-[10px] md:text-[11px] text-gray-400 mt-0.5 leading-none">{s.label}</p>
          </button>
        ))}
      </div>
      ...
      <div className="bg-white border border-gray-100 rounded-xl overflow-hidden">
        ...
        <div className="flex items-center gap-2 p-2.5">
          ...
          >
            ...
            Filtra
            {hasDropdownFilters && <span className="w-1.5 h-1.5 rounded-full bg-brand-500" />}
          </button>

          {/* Desktop dropdowns — always visible on md+ */}
          <div className="hidden md:flex items-center gap-2">
            {[
              { val: categoryId, set: setCategoryId, placeholder: 'Kategoria', opts: categories.map(c => ({ v: c.id, l: c.name_sq })) },
              ...(brands.length > 0 ? [{ val: brandId, set: setBrandId, placeholder: 'Brendi', opts: brands.map(b => ({ v: b.id, l: b.name })) }] : []),
              { val: audience, set: setAudience, placeholder: 'Audienca', opts: [{ v: 'home', l: '🏠 Shtëpi' }, { v: 'business', l: '🏢 Biznes' }, { v: 'both', l: '👥 Të Gjithë' }] },
              { val: status, set: setStatus, placeholder: 'Statusi', opts: [{ v: 'active', l: '✅ Aktiv' }, { v: 'inactive', l: '⛔ Joaktiv' }] },
              { val: stock, set: setStock, placeholder: 'Stoku', opts: [{ v: 'in', l: '✅ Në gjendje' }, { v: 'low', l: '⚠️ I ulët' }, { v: 'out', l: '❌ Pa gjendje' }] },
            ].map(({ val, set, placeholder, opts }) => (
              <div key={placeholder} className="relative">
                <select
                  value={val}
                  onChange={e => set(e.target.value)}
                  className={`appearance-none pl-2.5 pr-6 py-1.5 text-xs border rounded-lg focus:outline-none focus:ring-1 focus:ring-brand-400 cursor-pointer ${val ? 'border-brand-400 bg-brand-50 text-brand-700 font-semibold' : 'border-gray-200 bg-gray-50 text-gray-600'}`}
                >
            ...
            {[
              ...
              <div key={placeholder} className="relative">
                <select
                  value={val}
                  onChange={e => set(e.target.value)}
                  className={`w-full appearance-none pl-3 pr-7 py-2 text-xs border rounded-xl focus:outline-none cursor-pointer ${val ? 'border-brand-400 bg-brand-50 text-brand-700 font-semibold' : 'border-gray-200 bg-gray-50 text-gray-600'}`}
                >
                  <option value="">{placeholder}</option>
                  {opts.map(o => <option key={o.v} value={o.v}>{o.l}</option>)}
                </select>
        ...
        <div className="flex items-center gap-1.5 px-2.5 pb-2.5">
          {([
            ...
            <button
              ...
              className={`px-2.5 py-1 rounded-full text-[11px] font-semibold border transition-all ${
                state ? 'bg-brand-600 text-white border-brand-600' : 'bg-white text-gray-500 border-gray-200 hover:border-brand-300'
              }`}
            >
              {label}
            </button>
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            {hasFilters && (
              <button onClick={clearAll} className="flex items-center gap-1 text-[11px] font-semibold text-red-500 px-2 py-1 rounded-lg hover:bg-red-50 transition-colors">
                <X size={11} /> Pastro
              </button>
            )}
          </div>
      ...
      <div className="md:hidden space-y-2">
        {filtered.length === 0 ? (
          <div className="bg-white border border-gray-100 rounded-xl p-10 text-center text-gray-400">
            ...
            <p className="text-sm">{hasFilters ? 'Asnjë produkt nuk përputhet' : 'Nuk ka produkte'}</p>
            {hasFilters && <button onClick={clearAll} className="mt-1 text-xs text-brand-600">Pastro filtrat</button>}
          </div>
        ) : filtered.map(product => (
          <div key={product.id} className="bg-white border border-gray-100 rounded-xl p-3 flex items-center gap-3 active:bg-gray-50 transition-colors">
            {/* Image */}
            ...
            <div className="flex-1 min-w-0">
              <p className="font-semibold text-sm text-gray-900 leading-snug truncate">{product.name_sq}</p>
              <p className="text-[11px] text-gray-400 font-mono">{product.sku}</p>
              <div className="flex items-center gap-1.5 mt-1 flex-wrap">
                <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-md ${product.is_active ? 'bg-emerald-50 text-emerald-700' : 'bg-gray-100 text-gray-400'}`}>
                  {product.is_active ? 'Aktiv' : 'Joaktiv'}
                </span>
                {product.stock === 0
                  ? <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded-md bg-red-50 text-red-500">Pa gjendje</span>
                  : product.stock <= 10
                  ? <span className="text-[10px] font-semibold text-amber-600 flex items-center gap-0.5"><AlertTriangle size={9} />{product.stock}</span>
                  : null
                }
                {product.is_featured && <Star size={11} className="text-amber-400" fill="currentColor" />}
                {product.sale_price && <Tag size={10} className="text-red-400" />}
              </div>
            </div>
            ...
            <div className="flex-shrink-0 flex flex-col items-end gap-2">
              <div className="text-right">
                ...
                {product.sale_price && (
                  <p className="text-[11px] text-red-500 font-medium">{formatPrice(product.sale_price)}</p>
                )}
              </div>
              <Link
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50/80">
                {['Foto','Produkti','Kategoria','Brendi','Çmimi','Stoku','Audienca','Statusi',''].map(h => (
                  <th key={h} className={`text-[11px] font-semibold text-gray-400 uppercase tracking-wide px-3 py-2.5 ${h ? 'text-left' : ''}`}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-50">
              {filtered.map(product => (
                <tr key={product.id} className="hover:bg-gray-50/60 transition-colors group">
                  ...
                  <td className="px-3 py-2 text-xs text-gray-500">{product.category?.name_sq ?? <span className="text-gray-300">—</span>}</td>
                  <td className="px-3 py-2 text-xs text-gray-500">{product.brand?.name ?? <span className="text-gray-300">—</span>}</td>
                  <td className="px-3 py-2">
                    <p className="font-semibold text-sm text-gray-900">{formatPrice(product.price)}</p>
                    {product.sale_price && (
                      <p className="text-[11px] text-red-500 font-medium flex items-center gap-0.5"><Tag size={9} /> {formatPrice(product.sale_price)}</p>
                    )}
                  </td>
                  <td className="px-3 py-2">
                    <div className="flex items-center gap-1">
                      {product.stock === 0
                        ...
                        : product.stock <= 10
                        ? <span className="flex items-center gap-0.5 text-[11px] font-semibold text-amber-600"><AlertTriangle size={10} /> {product.stock}</span>
                  ...
                  <td className="px-3 py-2 w-16">
                    <div className="flex items-center gap-0.5 justify-end opacity-0 group-hover:opacity-100 transition-opacity">
                      <Link href={`/admin/products/${product.id}/edit`} onClick={saveScroll}
                        className="p-1.5 text-gray-400 hover:text-brand-600 hover:bg-brand-50 rounded-lg transition-all" title="Modifiko">
                        <Edit size={14} />
                      </Link>
              ...
              {filtered.length === 0 && (
                <tr>
                  <td colSpan={9} className="text-center py-12 text-gray-400">
                    <Package size={28} className="mx-auto mb-2 opacity-30" />
                    <p className="text-sm font-medium">{hasFilters ? 'Asnjë produkt nuk përputhet' : 'Nuk ka produkte ende'}</p>
                    {hasFilters && <button onClick={clearAll} className="mt-1 text-xs text-brand-600 hover:underline">Pastro filtrat</button>}
                  </td>
                </tr>
              )}
            </tbody>
