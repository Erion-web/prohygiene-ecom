import type { ReactNode } from 'react'
import { notFound } from 'next/navigation'
import { createClient } from '@/lib/supabase/server'
import { AdminHeader } from '@/components/admin/AdminHeader'
import { formatPrice, statusColor, statusLabel } from '@/lib/utils'
import Link from 'next/link'
import { ArrowLeft, Pencil, Truck } from 'lucide-react'
import Image from 'next/image'
import { OrderStatusUpdater } from './OrderStatusUpdater'
import { FalconSendButton } from './FalconSendButton'
import { DeleteOrderButton } from '../DeleteOrderButton'

async function getOrder(id: string) {
  const supabase = await createClient()
  const [orderRes, itemsRes] = await Promise.all([
    supabase.from('orders').select('*').eq('id', id).single(),
    supabase.from('order_items').select('*').eq('order_id', id),
  ])
  if (!orderRes.data) return null
  return { ...orderRes.data, items: itemsRes.data ?? [] }
}

function DetailRow({ label, children }: { label: string; children: ReactNode }) {
  return (
    <div className="flex items-start justify-between gap-3 text-sm">
      <span className="text-text-muted shrink-0">{label}</span>
      <span className="font-medium text-text-primary text-right min-w-0 break-words">{children}</span>
    </div>
  )
}

export default async function OrderDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params
  const order = await getOrder(id)
  if (!order) notFound()

  return (
    <div>
      <AdminHeader
        title={`Porosia ${order.order_number}`}
        subtitle={new Date(order.created_at).toLocaleString('sq-AL')}
        actions={
          <div className="flex items-center gap-1.5">
            <Link href={`/admin/orders/${order.id}/edit`} className="btn-ghost gap-1.5 text-sm">
              <Pencil size={15} />
              Modifiko
            </Link>
            <DeleteOrderButton
              id={order.id}
              orderNumber={order.order_number}
              redirectTo="/admin/orders"
              label="Fshi"
            />
            <Link href="/admin/orders" className="btn-ghost gap-1.5 text-sm">
              <ArrowLeft size={15} />
              Kthehu
            </Link>
          </div>
        }
      />

      <div className="admin-page grid lg:grid-cols-[minmax(0,1fr)_17.5rem] gap-4 items-start">
        <div className="admin-card p-5">
          <h3 className="admin-card-title mb-4">Artikujt e Porosisë</h3>
          <div className="space-y-3">
            {order.items.map((item: { id: string; product_image_url: string | null; product_name_sq: string; product_sku: string; quantity: number; unit_price: number; sale_price: number | null; subtotal: number }) => (
              <div key={item.id} className="flex items-center gap-4 p-3 bg-surface-soft rounded-xl">
                <div className="relative w-14 h-14 flex-shrink-0 rounded-xl overflow-hidden bg-white border border-surface-border">
                  {item.product_image_url ? (
                    <Image src={item.product_image_url} alt="" fill className="object-cover" sizes="56px" />
                  ) : (
                    <div className="absolute inset-0 flex items-center justify-center text-xl">🧴</div>
                  )}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="font-medium text-text-primary text-sm">{item.product_name_sq}</p>
                  <p className="text-xs text-text-muted font-mono">{item.product_sku}</p>
                </div>
                <div className="text-right text-sm flex-shrink-0">
                  <p className="font-bold">{formatPrice(item.subtotal)}</p>
                  <p className="text-text-muted text-xs">{item.quantity} × {formatPrice(item.sale_price ?? item.unit_price)}</p>
                </div>
              </div>
            ))}
          </div>

          <div className="border-t border-surface-border mt-4 pt-4 space-y-2">
            <div className="flex justify-between text-sm text-text-secondary">
              <span>Nëntotali</span>
              <span>{formatPrice(order.subtotal + order.discount_amount)}</span>
            </div>
            {order.discount_amount > 0 && (
              <div className="flex justify-between text-sm text-red-600">
                <span>Zbritja</span>
                <span>-{formatPrice(order.discount_amount)}</span>
              </div>
            )}
            <div className="flex justify-between text-sm text-text-secondary">
              <span>Transporti</span>
              <span>{order.shipping_cost === 0 ? 'Falas' : formatPrice(order.shipping_cost)}</span>
            </div>
            <div className="flex justify-between font-bold text-base pt-1 border-t border-surface-border">
              <span>TOTAL</span>
              <span>{formatPrice(order.total)}</span>
            </div>
          </div>
        </div>

        <aside className="admin-card space-y-4 p-4">
          <section className="space-y-2.5">
            <div className="flex items-center justify-between gap-2">
              <h3 className="admin-card-title">Statusi</h3>
              <span className={`badge border ${statusColor(order.status)}`}>
                {statusLabel(order.status, 'sq')}
              </span>
            </div>
            <OrderStatusUpdater orderId={order.id} currentStatus={order.status} />
          </section>

          <section className="space-y-2 border-t border-surface-border pt-4">
            <h3 className="admin-card-title">Klienti</h3>
            <div className="space-y-1.5">
              <DetailRow label="Emri">{order.customer_name}</DetailRow>
              <DetailRow label="Email">
                <a href={`mailto:${order.customer_email}`} className="text-brand-600">{order.customer_email}</a>
              </DetailRow>
              <DetailRow label="Tel">
                <a href={`tel:${order.customer_phone}`} className="text-brand-600">{order.customer_phone}</a>
              </DetailRow>
              <DetailRow label="Tipi">{order.customer_type === 'business' ? 'Biznes' : 'Individual'}</DetailRow>
              {order.business_name && <DetailRow label="Biznesi">{order.business_name}</DetailRow>}
              {order.fiscal_number && <DetailRow label="Nr. Fiskal"><span className="font-mono">{order.fiscal_number}</span></DetailRow>}
            </div>
          </section>

          <section className="space-y-2 border-t border-surface-border pt-4">
            <h3 className="admin-card-title">Dërgimi</h3>
            <div className="space-y-1.5">
              <DetailRow label="Qyteti">{order.city}</DetailRow>
              <DetailRow label="Adresa">{order.address}</DetailRow>
              {order.notes && <DetailRow label="Shënime"><span className="italic text-text-secondary font-normal">{order.notes}</span></DetailRow>}
            </div>
          </section>

          <section className="space-y-2 border-t border-surface-border pt-4">
            <h3 className="admin-card-title flex items-center gap-1.5">
              <Truck size={14} /> Falcon
            </h3>
            {order.falcon_order_id ? (
              <div className="space-y-1.5">
                <DetailRow label="ID"><span className="font-mono">{order.falcon_order_id}</span></DetailRow>
                {order.falcon_status_name && <DetailRow label="Statusi">{order.falcon_status_name}</DetailRow>}
              </div>
            ) : (
              <FalconSendButton orderId={order.id} />
            )}
          </section>

          <section className="space-y-2 border-t border-surface-border pt-4">
            <h3 className="admin-card-title">Pagesa</h3>
            <div className="space-y-1.5">
              <DetailRow label="Metoda">{order.payment_method === 'card' ? 'Kartë' : 'Kesh'}</DetailRow>
              <DetailRow label="Statusi">
                <span className={`badge border text-xs ${statusColor(order.payment_status)}`}>
                  {statusLabel(order.payment_status, 'sq')}
                </span>
              </DetailRow>
            </div>
          </section>
        </aside>
      </div>
    </div>
  )
}
